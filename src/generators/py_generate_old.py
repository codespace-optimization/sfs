...
def gen_py_strategist_idea_generation_instruction(num_ideas: int): 
    out = f"""Based on the function, feedback, and conclusions you drew, what are {num_ideas} ways in which you can fix the function so that it passes all the tests? Be as specific and concrete as possible, mentioning specific code blocks or helper functions that you will modify.  

Here's an example of what this might look like for 3 improvement ideas:

[example 3 improvement ideas]:

Thoughts: The feedback suggests that the main problem is that function is not taking into account the cards in either player's hand, which leads to failing the first test.
...
class PyGenerator(Generator):
    ...
    # def strategist_generate_ideas(self, feedback: str, num_ideas: int, model: ModelBase) -> str:
    #     return generic_strategist_generate_ideas(
