...
def generic_generate_internal_tests(
    ...
    all_tests = parse_tests(outputs[0])  # NOTE: outputs is a list of strings
...
def generic_generate_self_reflection(
    ...
    if model.is_chat:
        if self_reflection_few_shot is not None:
            ...
            reflection = model.generate_chat(messages=messages)[0]
...
def generic_strategist_generate_ideas(func_sig: str,
    model: ModelBase,
    strategy: str,
    prev_func_impl,
    feedback,
    ...
    simple_chat_instruction: str,
    code_block_instruction: str,
    idea_generation_instruction: Callable[[int], str],
    parse_ideas: Callable[[str], list[str]],
    add_code_block: Callable[[str], str]) -> list[str]:
    ...
    if model.is_chat:
        if strategy == "reflexion":
            ...
            messages = [
                ...
                Message(
                    role="user",
                    content=idea_generation_instruction(num_ideas),
                ),
            ]
            model_responses = model.generate_chat(messages=messages, num_comps=num_comps, temperature=temperature)
        else:
            system_prompt = f"{simple_chat_instruction}\n{code_block_instruction}"
            print_messages(system_prompt, func_sig)
            messages = [
            ...
            model_responses = model.generate_chat(messages=messages, num_comps=num_comps, temperature=temperature)
        return parse_ideas(model_responses[0])
    else:
        raise ValueError("Model is not chat based")
    
def generic_strategist_generate_strategy(func_sig: str,
    model: ModelBase,
    strategy: str,
    prev_func_impl: str,
    feedback: str,
    self_reflection: str,
    idea_to_implement: str, 
    num_comps: int,
    temperature,
    reflection_chat_instruction: str,
    reflection_few_shot: str,
    simple_chat_instruction: str,
    code_block_instruction: str,
    strategist_func_instruction: Callable[[str], str], 
    parse_code_block: Callable[[str], str],
    add_code_block: Callable[[str], str]) -> str:
    ...
    if model.is_chat:
        if strategy == "reflexion":
            ...
            messages = [
                ...
                Message(
                    role="user",
                    content=f"[idea to implement]: \n Here's 1 way to improve your code so that it passes the tests:\n{idea_to_implement} \n Incorporate this idea into your code as best as you can. \n\n[improved impl]:\n{func_sig}",
                ),
            ]
            model_responses = model.generate_chat(messages=messages, num_comps=num_comps, temperature=temperature)
        ...
        return parse_code_block(model_responses[0])
    else:
        raise ValueError("Model is not chat based")
